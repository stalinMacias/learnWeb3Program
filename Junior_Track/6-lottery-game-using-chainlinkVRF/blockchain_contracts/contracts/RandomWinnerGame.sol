...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
   ...
   * @param linkToken address of LINK token contract - The token in which the chainlink takes its payment
   * @param vrfFee the amount of LINK to send with the request - amount of link token that will be needed to send a randomness request
   ...
   */
    ...
    * @dev This function is onlyOwner which means that it can only be called by the owner. 
    * @dev It is used to start the game, after this function is called players can enter the game until limit has been achieved. 
    * @dev It also emits the GameStarted event.
    * @notice _entryFee is expressed in Wei
    */
    ...
    * @dev This function will be called when a user wants to enter a game. 
    * @dev If the maxPlayers limit is reached it will call the getRandomWinner function
    * @notice joinGame accepts ether and not WEI
    */
    ...
    function getPlayersJoined() public view returns (uint) {
    ...
    * @dev This function was inherited from VRFConsumerBase
    * @dev It is called by VRFCoordinator contract after it recieves the randomness from the external world.
    * @dev After recieving the randomness which can be any number in the range of uint256 we decrease its range from 0 to players.length-1 using the mod operaator
    * @dev This selects an index for us and we use that index to retrieve the winner from the players array
    * @dev It send all the ether in the contract to the winner and emits a GameEnded event
    * 
   ...
   */
    ...
    * @dev This function first checks if our contract has Link token before we request for randomness because chainlink contracts request fee in the form of Link token. 
    * @dev Then this function calls the requestRandomness which we inherited from VRFConsumerBase and begins the process for random number generation.
    */

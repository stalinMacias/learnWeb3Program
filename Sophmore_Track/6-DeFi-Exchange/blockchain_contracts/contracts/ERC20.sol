...
contract Exchange is ERC20 {
        ...
        returns (uint256, uint256)
    {
        ...
        payable(msg.sender).transfer(ethAmount); // For production code don't use the transfer() method to send ETH, instead, use the call{}() method
     ...
     * inputReserve and outputReserve would depend on which swap we are implementing
     * @return - Returns the minimum number of tokens that the user is willing to receive after the swap
     */
     ...
     * @param _minTokens - The minimum number of tokens that the user is willing to receive after the swap
     */
    function ethToCryptoDevToken(uint256 _minTokens) public payable {
        uint256 tokenReserve = getReserve(); // Get the total number of CryptoDev Tokens held by the contract

        // call the `getAmountOfTokens` to get the amount of Crypto Dev tokens that would be returned to the user after the swap
        // Notice that the `inputReserve` we are sending is equal to `address(this).balance - msg.value` instead of just `address(this).balance` , because `address(this).balance` already contains the `msg.value` user has sent in the given call, so we need to subtract it to get the actual input reserve
        uint256 tokensBought = getAmountOfTokens(
            msg.value, // inputAmount
            address(this).balance - msg.value, // inputReserve
            tokenReserve // outputReserve
        );
     ...
     * @param _tokensSold - The number of CryptoDev Tokens to be sold
     * @param _minEth - The minimum amount of ETH that the user is willing to receivein exchange for their tokens
     */
    function cryptoDevTokenToEth(uint256 _tokensSold, uint256 _minEth) public {
        ...
        uint256 ethBought = getAmountOfTokens(
            _tokensSold,  // inputAmount  
            tokenReserve, // inputReserve
            address(this).balance // outputReserve
        );
        ...
        payable(msg.sender).transfer(ethBought);  // Send to the user the calculated amount of ETH in exchange for their tokens
    }
}